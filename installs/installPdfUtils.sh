...
if [[ "$(getOsVers)" == "16.04" || "$(getOsVers)" == "18.04" || "$(getOsVers)" == "20.04" || "$(getOsVers)" == "22.04" || "$(getOsVers)" == "24.04" ]]; then
	...
	# HTML to PDF. wkhtmltopdf is no longer packaged for Ubuntu 26.04; WeasyPrint is the maintained repository replacement.
	sudo apt-get install --yes weasyprint
	# pdfsandwich is packaged directly by Ubuntu 26.04, so no manual SourceForge .deb is required.
